...
(def-esl-word call (push (esl-pop) *esl-return-stack*))
...
(def-esl-word if (let ((fb (esl-pop))
					   ...
					   (predval (esl-pop)))
				   (if predval 
					   (push tb *esl-return-stack*)
					 (push fb *esl-return-stack*))))
...
(defn_ esl-compile [esl-code]
  (let ((*esl-stack* esl-code))
	(loop with output = nil 
		  ...
		  (let ((part (esl-pop)))
			(cond
			 ((eq part nil) (eval `(function (lambda () (esl-push nil)))))
			 ((listp part)
...
(defun esl-next-part ()
  (let ((inst (pop (car *esl-return-stack*))))
	(if (not (car *esl-return-stack*))
		(pop *esl-return-stack*))
	inst))

(let ((*esl-return-stack* '( ( a ) (c d e f))))
  (print (esl-next-part))
  (print *esl-return-stack*))

(defun esl-eval-compiled (ccode)
  (push ccode *esl-return-stack*)
  (loop while *esl-return-stack* do

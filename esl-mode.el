...
;;
;; Documentation: See esl-mode (^HF esl-mode)
;;-------------------------------------------------------------------
...
display in).  Currently ignores esl-mode buffers but that may change
in the future."
...
but might be transparently changed in the future."
  (unless forth-idle-function-timer
    ;; install idle function only once (first time esl-mode is used)
...
This will disable hilighting of esl-mode buffers and will decrease
...
This eliminates the need for initially parsing esl-mode buffers and
...
(setq forth-words
	...
	((";" ";code" "end-word:") definition-ender (font-lock-keyword-face . 1))
...
(defvar forth-use-objects nil 
  "*Non-nil makes esl-mode also hilight words from the \"Objects\" package.")
...
(defvar forth-use-oof nil 
  "*Non-nil makes esl-mode also hilight words from the \"OOF\" package.")
...
(defvar forth-local-indent-words nil 
  "List of Forth words to prepend to `forth-indent-words', when a esl-mode
...
(defun forth-hack-local-variables ()
 ...
 esl-mode buffer. Prepend `forth-local-words' to `forth-words' and 
...
(defvar esl-mode-abbrev-table nil
  "Abbrev table in use in esl-mode buffers.")

(define-abbrev-table 'esl-mode-abbrev-table ())

(defvar esl-mode-map nil
  "Keymap used in Forth mode.")

(if (not esl-mode-map)
    (setq esl-mode-map (make-sparse-keymap)))

;(define-key esl-mode-map "\M-\C-x" 'compile)
(define-key esl-mode-map "\C-x\\" 'comment-region)
(define-key esl-mode-map "\C-x~" 'forth-remove-tracers)
(define-key esl-mode-map "\C-x\C-m" 'forth-split)
(define-key esl-mode-map "\e " 'forth-reload)
(define-key esl-mode-map "\t" 'forth-indent-command)
(define-key esl-mode-map "\C-m" 'forth-reindent-then-newline-and-indent)
(define-key esl-mode-map "\M-q" 'forth-fill-paragraph)
(define-key esl-mode-map "\e." 'forth-find-tag)

;; setup for C-h C-i to work
(eval-and-compile (forth-require 'info-look))
(when (memq 'info-look features)
  (defvar forth-info-lookup '(symbol (esl-mode "\\S-+" t 
						  (("(gforth)Word Index"))
						  "\\S-+")))
  ...
  (define-key esl-mode-map [?\C-h ?\C-i] 'info-lookup-symbol))
...
(defvar esl-mode-syntax-table nil
  "Syntax table in use in esl-mode buffers.")
...
;; help.
(if (not esl-mode-syntax-table)
    (progn
      (setq esl-mode-syntax-table (make-syntax-table))
      (let ((char 0))
	(while (< char ?!)
	  (modify-syntax-entry char " " esl-mode-syntax-table)
	  (setq char (1+ char)))
	(while (< char 256)
	  (modify-syntax-entry char "w" esl-mode-syntax-table)
	  (setq char (1+ char))))
      ))

(defun esl-mode-variables ()
  (set-syntax-table esl-mode-syntax-table)
  (setq local-abbrev-table esl-mode-abbrev-table)
...
Variables controlling syntax hilighting/recognition of parsed text:
 ...
 forth-custom-words
    ...
    in your .emacs.
 forth-local-words
    List of words to prepend to `forth-words', whenever a esl-mode
...
Variables controlling block-file editing:
 forth-show-screen
    ...
    This variable is by default nil for `esl-mode' and t for 
    `forth-block-mode'.
 ...
 forth-warn-long-lines
    ...
    t for `forth-block-mode' and to nil for `esl-mode'.
...
`run-forth'):
  ...
  inferior-esl-mode-hook
    Hook for customising inferior-esl-mode.
  forth-compile-command
    Default command to execute on `compile'.
" 
  (interactive)
  (kill-all-local-variables)
  (use-local-map esl-mode-map)
  (setq mode-name "Forth")
  (setq major-mode 'esl-mode)
  ...
  ;; if user switched from forth-block-mode to esl-mode, make sure the file
...
;      (run-forth forth-program-name))
  (run-hooks 'esl-mode-hook))

;;;###autoload
(define-derived-mode esl-block-mode esl-mode "Forth Block Source" 
  "Major mode for editing Forth block source files, derived from 
`esl-mode'. Differences to `esl-mode' are:
...
(define-key esl-mode-map "\C-x\C-e" 'compile)
(define-key esl-mode-map "\C-x\C-n" 'next-error)
(require 'compile)
...
;;       (append '((esl-mode . fume-function-name-regexp-forth) 
...
;;   (append '((esl-mode    . fume-find-next-forth-function-name))))
...
(when (memq 'comint features)
  ...
  (defcustom inferior-esl-mode-hook nil
    "*Hook for customising inferior-esl-mode."
    :type 'hook
    :group 'forth)

  (defvar inferior-esl-mode-map
    (let ((m (make-sparse-keymap)))
      (define-key m "\r" 'comint-send-input)
      (define-key m "\M-\C-x" 'forth-send-paragraph-and-go)
      (define-key m "\C-c\C-l" 'forth-load-file)
      m))
  ;; Install the process communication commands in the esl-mode keymap.
  (define-key esl-mode-map "\e\C-m" 'forth-send-paragraph-and-go)
  (define-key esl-mode-map "\eo" 'forth-send-buffer-and-go)

  (define-key esl-mode-map "\M-\C-x" 'forth-send-paragraph-and-go)
  (define-key esl-mode-map "\C-c\C-r" 'forth-send-region)
  (define-key esl-mode-map "\C-c\M-r" 'forth-send-region-and-go)
  (define-key esl-mode-map "\C-c\C-z" 'forth-switch-to-interactive)
  (define-key esl-mode-map "\C-c\C-l" 'forth-load-file)

  (defvar forth-process-buffer)

  (define-derived-mode inferior-esl-mode comint-mode "Inferior Forth"
    "Major mode for interacting with an inferior Forth process.

The following commands are available:
\\{inferior-esl-mode-map}

A Forth process can be fired up with M-x run-forth.

Customisation: Entry to this mode runs the hooks on comint-mode-hook and
inferior-esl-mode-hook (in that order).
...
\\[comint-continue-subjob] to continue it. "
    ;; Customise in inferior-esl-mode-hook
...
of `forth-program-name').  Runs the hooks `inferior-esl-mode-hook'
...
Then switch to the process buffer."
  ...
  (defcustom forth-source-modes '(esl-mode forth-block-mode)
...
processes.")
  ...
  (defun forth-proc ()
    ...
    (let ((proc (get-buffer-process (if (eq major-mode 'inferior-esl-mode)
					(current-buffer)
